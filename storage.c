...
load_fss_ext(uint64 fs, int fss, OkNNrdata *data, List **reloids, bool isSafe)
{
	if (isSafe && (!aqo_learn_statement_timeout || !lc_has_fss(fs, fss)))
		return load_aqo_data(fs, fss, data, reloids);
	else
	{
...
typedef enum {
	AQ_QUERYID = 0, AQ_LEARN_AQO, AQ_USE_AQO, AQ_FSPACE_HASH, AQ_AUTO_TUNING, 
	AQ_TOTAL_NCOLS
} aqo_queries_cols;
...
static size_t _compute_data_dsa(DataEntry *entry);
/*
...
_deform_queries_record_cb(void *data, size_t size)
{
	...
	entry = (QueriesEntry *) hash_search(stat_htab, &queryid, HASH_ENTER, &found);
	Assert(!found);
...
aqo_queries_load(void)
{
	...
	entries = hash_get_num_entries(queries_htab);
	Assert(entries == 0);
	data_load(PGAQO_STAT_FILE, _deform_queries_record_cb, NULL);
	/* Check existence of default feature space */
...
static size_t
_compute_data_dsa(DataEntry *entry)
{
...
load_aqo_data(uint64 fs, int fss, OkNNrdata *data, List **reloids)
{
	...
	ptr = (char *) dsa_get_address(data_dsa, entry->data_dp);
	Assert(ptr != NULL);
	Assert(key.fs == ((data_key *)ptr)->fs && key.fss == ((data_key *)ptr)->fss);
	ptr += sizeof(data_key);
	...
	Assert(ptr - (char *) dsa_get_address(data_dsa, entry->data_dp) ==
			_compute_data_dsa(entry));
end:
	LWLockRelease(&aqo_state->data_lock);
